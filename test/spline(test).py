...
def reject_pixels_3sigma(w, f):
    ...
    indices = np.where((flux_norm < top) & (flux_norm > bottom))
    w, flux_norm = w[indices], flux_norm[indices]
...
def get_image(wave, flux, flux_spl, flux_norm, filename):
    print ('+++ in get_image +++')
    plt.figure(figsize=(24, 12), dpi=75)
    p1 = plt.subplot(311)
    plt.plot(wave, flux, 'b-')
    plt.plot(wave, flux_spl, 'r-')
    #plt.plot(wave[points], flux[points], 'r.')
    plt.xlim(3700, 8600)
    plt.grid(True)
...
def traverse_spec(filename, filename_full):
    ...
    get_image(wave, flux, flux_spl, flux_norm, filename)
    return flux_spl
...
w, f = reject_pixels_3sigma(w, f)
print (len(w), len(f))

w, f = traverse_spec(w, f)
print (len(w), len(f))
...
childdir = 'LAMOST_DR4_snrg_gt_20_lt_100/'
filename = 'spec-56310-GAC078N51V2_sp14-191.fits'
filename_full = rootdir + childdir + filename
wave, flux1 = read_spec_la(filename_full)
flux = (flux1-min(flux1)) / (max(flux1)-min(flux1))
points = np.arange(0, len(flux), 1)
print (points)
f = traverse_spec(flux)
#f = traverse_spec(wave, flux/f)
get_image(f)
